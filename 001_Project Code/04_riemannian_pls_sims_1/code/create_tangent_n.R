create_tangent_n <- function(df, x_type, y_type, vars = "n", metric, height = 5,
  ...
  if(save){
    ggsave(glue::glue("../../002_Thesis/01_initial_draft/img/tangent_n_{metric}.png"),
           plot = p,
           height = height, width = 1.618 * height)
  }else if(save_git){
    ggsave(glue::glue("../../../../Riemannian-statistical-techniques-with-applications-in-fMRI/Model Recovery/Tangent PLS/fig/tangent_n_{x_type}_response_{x_type}_{metric}.png"),
           plot = p,
           height = height, width = 1.618 * height)
  }else{
    return(p)
