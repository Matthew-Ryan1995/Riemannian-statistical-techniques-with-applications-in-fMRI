...
#SBATCH --time=10:00:00    	                                # walltime allocation, which has the format (D-HH:MM:SS), here set to 1 hour
#SBATCH --mem=5GB         	                                # memory required per node (here set to 4 GB)
#SBATCH --output=job_outputs/sub/slurm-%A_%a.out
...
Rscript code/06_run_sub_sims.R
