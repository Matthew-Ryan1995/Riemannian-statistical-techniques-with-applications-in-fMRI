...
#'
#' @examples
generate_rpls_data <- function(n, x_dim, y_dim, sig, r, seed = NULL, L = 5,
  ...
  ## Create the scores
  Tmat1 <- matrix(rnorm(n * L/2, mean = r/(2*sqrt(L))), ncol = L)
  Tmat2 <- matrix(rnorm(n * L/2, mean = -r/(2*sqrt(L))), ncol = L)
...
#'
#' @examples
fit_riemannian_pls <- function(data, K = 1, tol = 1e-4, max.iter = 10, mc.cores = 1, method = NULL){
  model <- suppressWarnings(riemannian_pls(X = data$X, Y = data$Y, L = K,
                                           tol = tol, max.iter = max.iter, method = method,
                                           type_x = data$type_x, type_y = data$type_y,
                                           mc.cores = mc.cores, verbose = F))
  # model2 <- suppressWarnings(riemannian_pls(X = data$X, Y = data$Y, L = K,
  #                                          tol = tol, max.iter = max.iter, method = "tangent",
  #                                          type_x = data$type_x, type_y = data$type_y,
  #                                          mc.cores = mc.cores, verbose = F))
