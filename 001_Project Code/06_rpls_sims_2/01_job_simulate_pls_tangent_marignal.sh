...
#SBATCH --time=12:00:00    	                                # walltime allocation, which has the format (D-HH:MM:SS), here set to 1 hour
#SBATCH --mem=4GB         	                                # memory required per node (here set to 4 GB)
#SBATCH --output=job_outputs/tangent/slurm-%A_%a.out
...
Rscript code/01_run_tangent_marginal_sims.R
